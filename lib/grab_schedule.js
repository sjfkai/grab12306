...
var grabSchedule = function(global) {
	...
	logger.info("开始抓取车次列表");
        if (config.train_skip_grab_until != undefined) {
           logger.info("根据 config.train_skip_grab_until 跳过所有" + config.train_skip_grab_until + "之前的车次"  );
        }

	var stationListArr = global.stationList.data;
	var trainListArr = global.trainList.data;

          // Sort the train by name so it will be 3454,....,9234,D1,D2...,K1,K2,....,T1,....Z1,...
       trainListArr =   trainListArr.sort(function(t1,t2){
                       return t1.train.localeCompare(t2.train);
                  });
          logger.info(trainListArr);
	...
	return Promise.each(trainListArr, function(train) {
		++curr;
		//请求参数
                var train_name = train.train;
                // guard to not crawl train that has been downloaded before
                if (config.train_skip_grab_until != undefined) {
                     var firstLetter = train_name.charAt(0);
                     var charCode = firstLetter.charCodeAt(0);
                      // 'A' to 'Z' ?
                      if (charCode >= 65) {
                         // compare train name with config.train_skip_grab_until
                           var firstLetterSkip = config.train_skip_grab_until.charAt(0);
                           var charCodeSkip = firstLetterSkip.charCodeAt(0);
                           var skip = false;
                           if (charCodeSkip > charCode) {
                              skip = true;
                           } else if (charCodeSkip < charCode) {
                              skip = false;
                            } else {
                              // compare D123 to D2 for example
                              // compare the number
                              var numberSkip =  parseInt(config.train_skip_grab_until.substr(1));
                              var numberOfTrain =  parseInt(train_name.substr(1));
                              skip = numberOfTrain <= numberSkip;
                            }
                            if (skip) {
                              logger.info("跳过已经抓取的车次：" + train.train + ", 因为它在" + config.train_skip_grab_until + "之前");
                              return Promise.resolve();
                            }
                ...
                sleep(config.wait_time_between_downloads);
